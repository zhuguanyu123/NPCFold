...
class BlendedLoss(object):
    ...
    def calculate_loss2(self, target, output_embedding, output_cross_entropy=None):
            if target is not None:
                target = (target,)
            ...
            return cross_entropy_loss, loss_dict
...
class NPairLoss(nn.Module):
    ...
    def n_pair_loss(anchors, positives, negatives):
        ...
        # loss = torch.mean(torch.sigmoid_(x))
        # anchors = anchors.detach().cpu().numpy()
        # positives = positives.detach().cpu().numpy()
        # negatives = negatives.detach().cpu().numpy()
        # score1 = 0
        # score2 = 0
        ...
        #         # print(anchors[i])
        #         # print(negatives[i][j])
        #         score1 += calculate_similarity.calculate_pierxun(anchors[i],negatives[i][j])
        #         score2 += calculate_similarity.calculate_pierxun(anchors[i],positives[i])
        # loss = np.sum(np.exp(score1-score2))/negatives.shape[0]  # (n, 1)
        return loss
